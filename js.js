...
function displayMovements(acc, sort = false) {
  ...
  movs.forEach(function (value, i) {
    ...
    const operation = value > 0 ? "Зачисление" : "Списание";
...
function calcDisplaySum(movements) {
  ...
  const expences = movements
    .filter((mov) => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = `${Math.abs(expences)}` + "₽";
  labelSumInterest.textContent = `${incomes + expences}` + "₽";
}
...
//суммируем деньги на всех аккаунтах
